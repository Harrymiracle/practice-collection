<template>
    <div class="navbar">
        <nav class="nav-box clearfix">
            <ul class="nav-bar">
                <li v-for="(item, index) in itemTab" :key="item" class="nav-bar-item" :class="{'nav-bar-active':initIndex === index}" @click="changeTab(index)">
                    {{item.title}}
                </li>
            </ul>
        </nav>
        <section class="scroll-list">
            <travel v-for="(item,index) in travelsList" :key="item" :travel="item" v-show="initIndex == 0"></travel>
            <panel v-for="(item,index) in sportsList" :key="item" :sport="item" v-show="initIndex == 1"></panel>
        </section>
