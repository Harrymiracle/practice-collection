...
export default {
    [type.ADDEVENT](states, obj) {
        states.count++; //叠加store中的count
        obj.items.id = states.count; //把store中的count作为item的id
        states.event.unshift(obj.items);
        func.local.set(states); //实际项目中可能是向后台发请求（post），改变数据。
    },
    [type.EVENTDONE](states, obj) {
        for (let i = 0; i < states.event.length; i++) {
            if (states.event[i].id === obj.id) {
                ...
                // var item = states.event[i];//把刚刚做了改动的取出来，以便下面放在最前面。
    ...
    [type.CLEAREVENT](states) {
        states.event = []; //清空store
        func.local.clear(); //清空localStorage
    },
