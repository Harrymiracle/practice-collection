	...
	address._readyList = function(json, selectedAddress) {
		...
		$.each(json, function(i, v) {
			...
			if(i == 0 && !selectedAddress) {
				html.push('<li id='+v.id+' class="am-active">' + v.name + '</li>');
				index = i;
				return;
			}

			if(selectedAddress && (parseInt(v.id) === parseInt(selectedAddress))) {
				allname+=v.name;
				html.push('<li id='+v.id+' class="am-active">' + v.name + '</li>');
				index = i;
			} else {
				html.push('<li id='+v.id+' >' + v.name + '</li>');
			}
		});
		$("input[name=district]").val(allname==""?"":allname);
		return {
			list: html.join(''),
			index: index
		};
	...
	address.prototype._selectEnd = function() {
		...
		if(self.options.selectNumber != 1) { // selectNumber = 1 表示只选省级
			...
			if(cityJson.district.length && self.options.selectNumber != 2) { // selectNumber = 2 表示只选省市级
				...
				sJson.district_id = districtJson.id;
			}
		}
		...
		if(!self.options.customOutput) {
			self.$element.find("input[type=text]").val(sJson.prov + sJson.city + sJson.district);
			self.$element.find("input[name=text]").val(sJson.prov +','+ sJson.city +','+ sJson.district);
			self.$element.find("input[type=hidden]").val(sJson.prov_id +','+ sJson.city_id +','+ sJson.district_id);
		}
